...
/**
 * `auto_solds` — read from the live database, which had no migration for it.
 *
...
return new class extends Migration
{
    ...
    {
        if (Schema::hasTable('auto_solds')) {
            return;
        }

        Schema::create('auto_solds', function (Blueprint $table) {
            $table->bigInteger('id', true);
            $table->bigInteger('user_id')->nullable();
            $table->bigInteger('post_id')->nullable();
            $table->string('buyer_name', 100)->nullable();
            $table->date('date')->nullable();
            $table->string('sold_amount', 100)->nullable();
    ...
    {
        Schema::dropIfExists('auto_solds');
    }
};
