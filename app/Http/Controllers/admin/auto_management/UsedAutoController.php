...
class UsedAutoController extends Controller
{
    public function __construct()
    {
        $this->middleware(['permission:used_auto'])->only(['index']);
        $this->middleware(['permission:add_used_auto'])->only(['create','store']);
        $this->middleware(['permission:edit_used_auto'])->only(['edit','update']);
        $this->middleware(['permission:sell_used_auto'])->only(['sell_auto']);
        $this->middleware(['permission:delete_used_auto'])->only(['delete']);
        $this->middleware(['permission:view_details_used_auto'])->only(['viewDetails']);
    ...
    public function index()
    {
        ...
        $used_autos = Auto::UsedAuto()->where('auto_status','active')
        ...
        return view('admin.auto_management.used_auto.index',compact('used_autos'));
    }
    ...
    public function viewDetails($id)
    {
        ...
        return view('admin.auto_management.used_auto.view',compact('auto'));
    }
    ...
    public function create()
    {
        ...
        return view('admin.auto_management.used_auto.create',compact('brands','models','fuels','owners'));
    }
    ...
    public function store(Request $request)
    {
        try {
            ...
            return redirect()->route('auto-management.used-auto')->with('success','Auto details Created successfully!');
        } catch (\Illuminate\Validation\ValidationException $e) {
            return redirect()->route('auto-management.used-auto.create')
                 ->withErrors($e->errors());
        } catch (\Exception $e) {
            return redirect()->route('auto-management.used-auto.create')
    ...
    public function edit($id)
    {
        ...
        $fuels  = AutoFuelType::where('status',1)->get(['id','name']);
        $owners = AutoOwners::where('status',1)->get(['id','name']);
        return view('admin.auto_management.used_auto.edit',compact('brands','models','fuels','owners','auto'));
    }
    ...
    public function update(Request $request)
    {
        try {
            ...
            return redirect()->route('auto-management.used-auto')->with('success','Auto details Updated successfully!');
        } catch (\Illuminate\Validation\ValidationException $e) {
            return redirect()->route('auto-management.used-auto.update')
                 ->withErrors($e->errors());
        } catch (\Exception $e) {
            return redirect()->route('auto-management.used-auto.update')
                 ->withErrors($e->getMessage());
    ...
    public function sell_auto(Request $request)
    {
        try {
            ...
            if ($auto->auto_status === 'sold') {
                return redirect()->route('auto-management.used-auto')
            ...
            if ($sold_auto) {
                ...
                $auto->save();
                return redirect()->route('auto-management.used-auto')->with('success','Auto has been marked as sold successfully!');
            }  else{
                return redirect()->route('auto-management.used-auto')
