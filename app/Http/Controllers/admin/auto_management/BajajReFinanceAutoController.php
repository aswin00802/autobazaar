...
class BajajReFinanceAutoController extends Controller
{
    public function __construct()
    {
        $this->middleware(['permission:bajaj_refinance_auto'])->only(['index']);
        $this->middleware(['permission:add_bajaj_refinance_auto'])->only(['create','store']);
        $this->middleware(['permission:edit_bajaj_refinance_auto'])->only(['edit','update']);
        $this->middleware(['permission:sell_bajaj_refinance_auto'])->only(['sell_auto']);
        $this->middleware(['permission:delete_bajaj_refinance_auto'])->only(['delete']);
        $this->middleware(['permission:view_details_bajaj_refinance_auto'])->only(['viewDetails']);
    ...
    public function index()
    {
        ...
        return view('admin.auto_management.bajaj_refinance.index',compact('used_autos'));
    }
    ...
    public function viewDetails($id)
    {
        ...
        return view('admin.auto_management.bajaj_refinance.view',compact('auto'));
    }
    ...
    public function create()
    {
        ...
        return view('admin.auto_management.bajaj_refinance.create',compact('brands','models','fuels','owners'));
    }
    ...
    public function store(Request $request)
    {
        try {
            ...
            return redirect()->route('auto-management.bajaj-refinance-auto')->with('success','Bajaj Re Finance Auto details Created successfully!');
        } catch (\Illuminate\Validation\ValidationException $e) {
            return redirect()->route('auto-management.bajaj-refinance-auto.create')
                 ->withErrors($e->errors());
        } catch (\Exception $e) {
            return redirect()->route('auto-management.bajaj-refinance-auto.create')
    ...
    public function edit($id)
    {
        ...
        $fuels  = AutoFuelType::where('status',1)->get(['id','name']);
        $owners = AutoOwners::where('status',1)->get(['id','name']);
        return view('admin.auto_management.bajaj_refinance.edit',compact('brands','models','fuels','owners','auto'));
    }
    ...
    public function update(Request $request)
    {
        try {
            ...
            return redirect()->route('auto-management.bajaj-refinance-auto')->with('success','Bajaj Re Finance Auto details Updated successfully!');
        } catch (\Illuminate\Validation\ValidationException $e) {
            return redirect()->route('auto-management.bajaj-refinance-auto.update')
                 ->withErrors($e->errors());
        } catch (\Exception $e) {
            return redirect()->route('auto-management.bajaj-refinance-auto.update')
                 ->withErrors($e->getMessage());
    ...
    public function delete(Request $request)
    {
        ...
        if ($auto) {
            ...
            $auto->save();
            return response()->json(array('success' => true, 'message' => 'Bajaj Re Finance Auto Deleted Successfully'));
        } else {
            return response()->json(array('success' => false, 'message' => 'Bajaj Re Finance Auto Not Found?...'));
        }
    }
    ...
    public function sell_auto(Request $request)
    {
        try {
            ...
            if ($auto->auto_status === 'sold') {
                return redirect()->route('auto-management.bajaj-refinance-auto')
            ...
            if ($sold_auto) {
                ...
                $auto->save();
                return redirect()->route('auto-management.bajaj-refinance-auto')->with('success','Auto has been marked as sold successfully!');
            }  else{
                return redirect()->route('auto-management.bajaj-refinance-auto')
