...
<div class="row mt-5">
    @can('add_sparepart_product')
        <div class="col-12 col-sm-12 col-md-12 col-lg-12 mb-3 d-flex justify-content-end">
            <a href="{{route('spare-parts.product.create')}}" class="btn create-new btn-primary">
                <span>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if(isset($product->image))
                                            <img src="{{asset($product->image)}}" alt="image" class="img-fluid" style="width:80px;height:80px">
                                        @endif
                                    </td>
                                    <td>
                                        @can('edit_sparepart_product')
                                            <a href="{{ route('spare-parts.product.edit',$product->id) }}" class="btn btn-sm btn-text-secondary rounded-pill btn-icon item-edit"><i class="icon-base ri ri-edit-box-line icon-20px"></i></a>
                                        @endcan
                                        @can('delete_sparepart_product')
                                            <a class="btn btn-sm btn-text-danger text-danger rounded-pill btn-icon item-delete" data-id="{{$product->id}}"><i class="icon-base ri ri-delete-bin-2-line icon-20px"></i></a>
                                        @endcan
                                    </td>
                                </tr>
                            @endforeach
        ...
        $(document).on("click", ".item-delete", function() {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        type:'POST',
                        url:"{{route('spare-parts.product.delete')}}",
                        data: {
                            "_token": "{{ csrf_token() }}",
                            id:id,
                        },
