                                ...
                                @if(!empty($refinances))
                                    @foreach($refinances as $refinance)
                                        <tr>
                                            ...
                                            <td>{{$refinance->user->name ?? 'Not Available'}}</td>
                                            <td>{{$refinance->user->phone_number ?? 'Not Available'}}</td>
                                            <td>{{$refinance->brand->brand_name ?? 'Not Available'}}</td>
                                            <td>{{$refinance->specific_model ?? 'Not Available'}}</td>
                                            <td>{{$refinance->registration_year ?? 'Not Available'}}</td>
                                            <td>{{$refinance->registration_number ?? 'Not Available'}}</td>
                                            <td>
                                                @if($refinance->status == 1)
                                                    <span class="badge rounded-pill text-bg-danger">Requested</span>
                                                @elseif($refinance->status == 2)
                                                    <span class="badge rounded-pill text-bg-danger">Pending</span>
                                                @elseif($refinance->status == 3)
                                                    <span class="badge rounded-pill text-bg-secondary">Contacted</span>
                                                @elseif($refinance->status == 4)
                                                    <span class="badge rounded-pill text-bg-success">Follow Up</span>
                                                @elseif($refinance->status == 5)
                                                    <span class="badge rounded-pill text-bg-success">Approved</span>
                                                @elseif($refinance->status == 6)
                                            ...
                                            <td>
                                                @can('re_finance_update')
                                                    <button type="button" class="btn btn-primary status_update_btn" data-status="{{$refinance->status}}" data-id ="{{$refinance->id}}"><i class="icon-base ri ri-file-edit-line icon-20px"></i></button>
                                                @endcan
                                            </td>
                                            <!-- <td>
                                                <a href="{{ route('services.gas-station.edit',$refinance->id) }}" class="btn btn-sm btn-text-secondary rounded-pill btn-icon item-edit"><i class="icon-base ri ri-edit-box-line icon-20px"></i></a>
                                                <a class="btn btn-sm btn-text-danger text-danger rounded-pill btn-icon item-delete" data-id="{{$refinance->id}}"><i class="icon-base ri ri-delete-bin-2-line icon-20px"></i></a>
                                            </td> -->
                                        </tr>
                                    @endforeach
                                @endif
        ...
        <div class="modal-content">
            <form method="post" name="auto_enquiry_status_update" id="auto_enquiry_status_update">
                ...
                <div class="modal-header">
                    <h5>Auto Re Finance Status</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <input type="hidden" id="table_id" name="table_id">
                    <small class="fw-medium">Re Finance Status</small>
...
<script type="text/javascript">
    $(document).ready(function(){
        $(document).on("click", ".item-delete", function() {
            ...
            Swal.fire({
                title: 'Are you sure?',
                text: "You won't be able to delete this Auto Re Finance!",
                icon: 'warning',
        ...
        $("#auto_enquiry_status_update").on("submit", function (e) {
            ...
            $.ajax({
                type:'POST',
                url:"{{route('services.re-finance.status-update')}}",
                data: {
                    "_token": "{{ csrf_token() }}",
                    id:id,
                    status:status,
