...
<div class="row">
    <div class="col-12 col-sm-12 col-md-12 col-lg-12 mb-3">
        <a href="{{route('auto-management.used-auto')}}" class="btn btn-secondary" style="float:right">Back</a>
    </div>
    ...
    <div class="col-xl-8 col-lg-7 col-md-7 order-0 order-md-1">
        <div class="row">
            <div class="col-12 col-sm-12 col-md-12 col-lg-12 mb-2">
                <div class="card">
                    <h4 class="card-header text-center">Used Auto Details - {{$auto->auto_unique_id??''}}</h4>
                </div>
            </div>
            @can('user_post_auto_approval')
            @if($auto->post_type == 'U' && $auto->status == 2)
            <div class="col-12 col-sm-12 col-md-12 col-lg-12 mb-2">
                <div class="card">
                    <h4 class="card-header text-center">User Post Auto Approval</h4>
                    <div class="card-body">
                        <form action="{{ route('user-management.users-post-auto-approval') }}" method="post">
                            @csrf
                            <input type="hidden" name="id" value="{{ $auto->id }}">
                            <div class="row">
                                <div class="col-12 col-sm-4 col-md-4 col-lg-4">
                                    <div class="mb-4">
                                        <label class="form-label" for="">Select Auto Status</label>
                                        <select class="form-select" required name="status" id="status">
                                            <option value="">Choose Auto Status</option>
                                            <option value="1">Active</option>
                                            <option value="3">Reject</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="col-12 col-sm-4 col-md-4 col-lg-4 d-none" id="resion">
                                    <div class="mb-4">
                                        <label class="form-label" for="">Remark</label>
                                        <textarea class="form-control h-px-100" name="remark" id="remark" placeholder="Auto Reject Remark here..."></textarea>
                                    </div>
                                </div>
                                <div class="col-12 col-sm-4 col-md-4 col-lg-4">
                                    <label class="form-label" for="">Auto Status Submit</label><br>
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            $('#status').on('change',function(){
                $('#resion').removeClass('d-block').addClass('d-none')
                let status = $('#status').val()
                if(status == 3){
                    $('#resion').removeClass('d-none').addClass('d-block')
                } else {
                    $('#resion').removeClass('d-block').addClass('d-none')
                }
            })
