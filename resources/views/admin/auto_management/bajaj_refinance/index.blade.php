...
@section('title')
Auto Management / Bajaj ReFinance Auto
@endsection
...
@section('content')
<div class="row">
    @can('add_bajaj_refinance_auto')
        <div class="col-12 col-sm-12 col-md-12 col-lg-12 mb-3 d-flex justify-content-end">
            <a href="{{route('auto-management.bajaj-refinance-auto.create')}}" class="btn create-new btn-primary">
                <span>
                    <span class="d-flex align-items-center">
                        <i class="icon-base ri ri-add-line icon-18px me-sm-1"></i>
                        <span class="d-none d-sm-inline-block">Add Bajaj ReFinance Auto</span>
                    </span>
                </span>
            </a>
        </div>
                        ...
                        @if(!empty($used_autos))
                            @foreach($used_autos as $auto)
                                <tr>
                                    ...
                                    <td>
                                         @can('edit_bajaj_refinance_auto')
                                            <a href="{{ route('auto-management.bajaj-refinance-auto.edit',$auto->id) }}" class="btn btn-sm btn-text-secondary rounded-pill btn-icon item-edit"><i class="icon-base ri ri-edit-box-line icon-20px"></i></a>
                                        @endcan
                                        @can('delete_bajaj_refinance_auto')
...
<div class="modal fade" id="auto_sell_model" data-bs-backdrop="static" tabindex="-1">
    <div class="modal-dialog">
        <form class="modal-content" action="{{ route('auto-management.bajaj-refinance-auto.sell') }}" name="sell_form" id="sell_form" method="post">
            @csrf
        ...
        $(document).on("click", ".item-delete", function() {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        type:'POST',
                        url:"{{route('auto-management.bajaj-refinance-auto.delete')}}",
                        data: {
                            "_token": "{{ csrf_token() }}",
                            id:id,
                        },
