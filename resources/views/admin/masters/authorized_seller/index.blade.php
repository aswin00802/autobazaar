...
<div class="row">
    <div class="col-md-12">
        <div class="row">
            @can('add_authorized_seller')
            <div class="col-12 col-sm-12 col-md-12 col-lg-12 mb-3 d-flex justify-content-end">
                <a href="{{route('masters.auto-authorized-seller.create')}}" class="btn create-new btn-primary">
                    <span>
                        <span class="d-flex align-items-center">
                            <i class="icon-base ri ri-add-line icon-18px me-sm-1"></i>
                            <span class="d-none d-sm-inline-block">Add New Dealer</span>
                        </span>
                    </span>
                </a>
            </div>
                                ...
                                @if(!empty($sellers))
                                    @foreach($sellers as $seller)
                                        <tr>
                                            ...
                                            <td>{{ $seller->brand?->brand_name ?? null }}</td>
                                            <td>{{ $seller->dealer_name ?? null }}</td>
                                            <td>{{ $seller->dealer_type ?? null}}</td>
                                            <td>{{ $seller->location ?? null }}</td>
                                            <td>{{ $seller->address ?? null }}</td>
                                            <td>{{ $seller->contact ?? null}}</td>
                                            <td>
                                                @can('edit_authorized_seller')
                                                    <a href="{{ route('masters.auto-authorized-seller.edit',$seller->id) }}" class="btn btn-sm btn-text-secondary rounded-pill btn-icon item-edit"><i class="icon-base ri ri-edit-box-line icon-20px"></i></a>
                                                @endcan
                                                @can('delete_authorized_seller')
        ...
        $(document).on("click", ".item-delete", function() {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        type:'POST',
                        url:"{{route('masters.auto-authorized-seller.delete')}}",
                        data: {
                            "_token": "{{ csrf_token() }}",
                            id:id,
                        },
